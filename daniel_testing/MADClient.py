...
def execute_on_server(server_port_tuple, argument, requirements = None):
	...
	if mad_frame_server:
		...
		outputsize = frame_data[2]

		if requirements == None:	# Non-compiling action executed.
			output = sd.recv(outputsize).decode("utf-8")
			print(CYN)
			print(f"{CYN} {'Exit output:'} {output}")

		else:						# Compiling action executed.
			outputname = sd.recv(frame_data[1]).decode("utf-8")
			output = sd.recv(outputsize)
...
def get_cost_from_server(server_port_tuple):
	...
	reply = sd.recv(4)
	if reply:
		reply_cost = struct.unpack('i', reply)[0]

		print('get cost, cost is', reply_cost) ###DEBUG
		return reply_cost
	return -1
