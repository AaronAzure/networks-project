	...
	while True:
		...
		while True:
			...
			if data: 
				...
				if data[0] == '1':
					cost_no = cost_for_execution()
					cost = struct.pack('i', cost_no)
					print('cost is', sys.getsizeof(cost))
					client.send(cost)
					continue
				...
				if requirements == []:	# Non-compiling action executed.
					header = struct.pack('i i i', int(execution.returncode), 0, sys.getsizeof(execution.stdout))
					print("Header with a non-file output:", header)
					print("Funny header size:", struct.calcsize('iii'))
					client.send(header)
					client.send(execution.stdout)
					 
				else:
				#########
					return_code = int(execution.returncode)
					...
					header = struct.pack('i i i', return_code, filenamelength, filesize)
